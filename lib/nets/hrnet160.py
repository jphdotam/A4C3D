                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    nn.ReLU(True),
                                    BlurPool(channels=num_outchannels_conv3x3, filt_size=4, stride=2)))
                        else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        num_input = cfg['DATASET']['NUM_INPUT_CHANNELS']
        self.final_sigmoid_needed = extra.get('SIGMOID', False)
        self.onnx_output = cfg.get('ONNX_OUTPUT', False)

        self.conv1 = nn.Conv2d(num_input, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.blurpool1 = BlurPool(channels=64, filt_size=4, stride=2)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn2 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.blurpool2 = BlurPool(channels=64, filt_size=4, stride=2)
        self.relu = nn.ReLU(inplace=True)
        ...
        self.deconv_config = extra['DECONV']

    def _make_final_layers(self, cfg, input_channels):
        dim_tag = cfg['DATASET']['NUM_CLASSES']
        extra = cfg['MODEL']['EXTRA']

        final_layers = []
        output_channels = cfg['DATASET']['NUM_CLASSES']
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg['NUM_DECONVS']):
            if deconv_cfg['CAT_OUTPUT'][i]:
                final_output_channels = cfg['DATASET']['NUM_CLASSES']
...
def get_2dnet_cfg(cfg):
    ...
    out['MODEL'] = {}
    out['DATASET'] = {}
    out['DATASET']['NUM_INPUT_CHANNELS'] = 3  # prev 3 * 3  # Matt has space for frame before & after, so 3 sets of 3

    with open(cfg['paths']['keys_json'], "r") as read_file:
        keypoint_names = list(json.load(read_file).keys())

    out['DATASET']['NUM_CLASSES'] = len(keypoint_names)

    out['MODEL']['EXTRA'] = NET_CFG
    out['MODEL']['PRETRAINED'] = False
