...
class E32Dataset(Dataset):
    ...
    def load_label(self, labelpath, frame_from, frame_to):
        ...
        if format == 'npz':
            ...
            label = np.zeros(len(labelpath), labelpath)
    ...
    def get_labels(self):
        ...
        for studypath, studydict in tqdm(self.studies.copy().items()):
            ...
            if format == 'npz':
                ...
                labels = sorted(glob(os.path.join(studypath, f"label_{self.labelname}_*.png")))
                if not len(labels):
                    print(f"Failed to find labels for {studypath}")
                    del self.studies[studypath]
                    continue

                template = f"label_{self.labelname}_" + "_[0-9]{4}_([0-9]{4})_[0-9]{3}_([0-9]{3}).png"
                n_frames, n_labels = re.findall(template, labels[0])[0]  # Returns a list of len 1, with a tuple of 2

                labelpaths = []
                if (n_frames * n_labels) != len(labels):
                    print(f"Found {len(labels)} for {studypath} but expected {n_frames * n_labels} ({n_frames} frames, {n_labels} labels)")
                    del self.studies[studypath]
                    continue
                else:
                    self.studies[studypath]['label'] = labelpaths
    ...
    def generate_label_from_pngs(self, pngpaths, studypath):
            ...
            with torch.no_grad():
                try:
                    if self.cfg['data']['2d']['multi_res']:
                        ...
                        scale_factors = (4,2) if self.cfg['data']['2d']['multi_res']
        ...
        if label_format == 'npz':
            ...
            label = y_batches[:, output_layer_ids]
            label = label - label.min()
            label = label / label.max()
            label = (label*255).astype(np.uint8)
            n_frames, n_layers, height, width = label.shape
            for i_frame, frame in enumerate(label):
                for i_layer, layer in enumerate(frame):
                    pngpath = os.path.join(studypath, f"label_{self.labelname}_{i_frame:04d}_{n_frames:04d}_{i_layer:03d}_{n_layers:03d}.png")
                    skimage.io.imsave(pngpath, layer, check_contrast=False)
