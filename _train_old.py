...
if __name__ == "__main__":
    ...
    use_kldiv = cfg['training']['train_criterion'] == 'kldivloss'
    ...
    for epoch in range(starting_epoch, n_epochs + 1):
        if local_rank == 0:
            ...
            # Vis seg
            vis_mse(ds_test, model, epoch, cfg, kldiv=use_kldiv)

        # Cycle
        train_loss = cycle('train', model, dl_train, epoch, train_criterion, optimizer, cfg, scheduler, kldiv=use_kldiv, local_rank=local_rank)
        test_loss = cycle('test', model, dl_test, epoch, test_criterion, optimizer, cfg, scheduler, kldiv=use_kldiv, local_rank=local_rank)
